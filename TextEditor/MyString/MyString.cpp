...
long long MyString::find(const char* line_to_find, unsigned int index)
{
    ...
    std::size_t* suffix_table = new std::size_t[line_size];
    ...
    {
        std::size_t offset = line_size;
        while (offset && !suffix_match(line_to_find, line_size, offset, i))
            --offset;
        suffix_table[line_size - i - 1] = line_size - offset;
    }

    for (std::size_t this_pos = index; this_pos <= this->size() - line_size; )
    {
        std::size_t template_idx = line_size - 1;
        ...
        /*  not similar  */
        this_pos += max_own_realization(suffix_table[template_idx], template_idx - stop_table[(int)this->value_[template_idx + this_pos]]);
        /*          ^^^         ^^^^               */
        /*         suffix     stop-symbol          */
    }
    delete[] suffix_table;
...
std::size_t max_own_realization(std::size_t a, std::size_t b)
{
    return a > b ? a : b;
}

// for suffix table creating
int suffix_match(const char* line_to_find, std::size_t line_size, std::size_t offset, std::size_t suffixlen)
{
    if (suffixlen < offset)
